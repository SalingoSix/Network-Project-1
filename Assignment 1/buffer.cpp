...
void buffer::retrieveData(char** sending, int* length)
{
	if (mReadIndex == mWriteIndex)
		return;
	int index = 0;
	*length = mWriteIndex - mReadIndex;
	char* pullArray = new char[*length];
	for (; mReadIndex < mWriteIndex; mReadIndex++, index++)
	{
		pullArray[index] = (char)mBuffer[mReadIndex];
		std::cout << (char)mBuffer[mReadIndex] << std::endl;
	}
